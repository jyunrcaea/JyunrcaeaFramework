...
public class TextTexture : Texture
{
    ...
    public void Resize(int size)
    {
        ...
        if (SDL_ttf.TTF_SetFontSize(Font.pointer , this.Size = size) != 0)
            throw new JyunrcaeaFrameworkException($"글꼴 크기를 조정하는데 실패하였습니다. (SDL Error: {SDL.SDL_GetError()})");
    }
    ...
    internal void Rendering()
    {
        ...
        if (this.texture == IntPtr.Zero)
            throw new JyunrcaeaFrameworkException($"렌더링 된 텍스트를 텍스쳐로 변환하는데 실패하였습니다. {SDL.SDL_GetError()}");
